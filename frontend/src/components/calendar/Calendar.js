    ...
    return (
      ...
      <div className="App" style={{ paddingBottom: 20, paddingTop: 20 }}>
        ...
        <FullCalendar
          ...
          dayMaxEvents={true}
          datesSet={(args) => {
            let monthDate = ("###datesSet:", args).startStr.substring(0, 10);
            let startMonth = parseInt(monthDate.substring(5, 7), 10);
            let endDate = ("###datesSet:", args).endStr.substring(0, 10);
            let endMonth = parseInt(endDate.substring(5, 7), 10);
            //0123456789
            let correctMonth = parseInt(monthDate.substring(5, 7), 10);
            if ((endMonth - startMonth > 1) || (endMonth - startMonth < -1)) {
              correctMonth = correctMonth + 1;
            }
            let correctMonthBefore = (correctMonth - 1);
            let correctMonthAfter = (correctMonth + 1);
            let correctYear = monthDate.substring(0, 4);
            let yearAfter = parseInt(correctYear, 10) + 1;
            let yearBefore = yearAfter - 2;
            let neededBefore = false;
            let needed = false;
            let neededAfter = false;
            ...
            let monthDateBefore = ""
            if (neededBefore) {
              monthDateBefore = yearAfter + "-" + correctMonthBefore + "-01";
            }
            if (neededBefore) {
              monthDateBefore = yearBefore + "-" + correctMonthBefore + "-01";
            }
            else {
              monthDateBefore = correctYear + "-" + correctMonthBefore + "-01";
            }
            ...
            let monthDateAfter = ""
            if (neededAfter) {
              monthDateAfter = yearAfter + "-" + correctMonthAfter + "-01";
            }
            else {
              monthDateAfter = correctYear + "-" + correctMonthAfter + "-01";
            }
            console.log(monthDateBefore, monthDate, monthDateAfter);
